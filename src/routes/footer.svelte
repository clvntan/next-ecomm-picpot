<div data-theme="dim">
	<footer class="py-8">
		<div class="max-w-7xl mx-auto px-4">
			<div class="flex flex-wrap -mx-4 my-4">
				<!-- Left Column -->
				<div class="w-80% md:w-1/2 px-4 mb-4 md:mb-0">
					<img src="/Picpot_logo_iri.png" alt="Logo" class="w-auto h-16 mb-2" />
					<p class="text-base pl-4">The best stock photos shared by creators.</p>
					<p class="text-xs font-thin pl-4">&copy; PicPot, 2024. All rights reserved.</p>
				</div>

				<div class="w-20% md:w-1/2 px-4">
					<div class="flex space-x-4 mt-6 pl-4 justify-end">
						<a href="#" target="_blank" rel="noopener noreferrer">
							<!-- Facebook Icon -->
							<img src="/facebook.svg" alt="Facebook" class="w-7 h-7" />
						</a>
						<a href="#" target="_blank" rel="noopener noreferrer">
							<!-- Instagram Icon -->
							<img src="/instagram.svg" alt="Instagram" class="w-7 h-7" />
						</a>
						<a href="#" target="_blank" rel="noopener noreferrer">
							<!-- Pinterest Icon -->
							<img src="/pinterest.svg" alt="Pinterest" class="w-7 h-7" />
						</a>
						<a href="#" target="_blank" rel="noopener noreferrer">
							<!-- Github Icon -->
							<img src="/github.svg" alt="Github" class="w-7 h-7" />
						</a>
					</div>
				</div>
			</div>
		</div>
	</footer>
</div>
